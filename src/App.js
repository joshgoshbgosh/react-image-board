...
class ImageBoard extends Component {
  ...
  render() {
let images = this.state.images.map((image,index)=> {return <ImageItem key={index} image={image}/>});
    return(
      <div>
...
render() {
  ...
  return(<div><form>
    <input type="text" name="url" value={this.state.url}onChange={this.handleInput}/><button type="submit" className="btn btn-primary">Submit</button></form></div>)
...
render(){
  return
  (<div><img src={this.props.image.url}/><div>{this.props.image.caption}</div></div>
  );
}
}
